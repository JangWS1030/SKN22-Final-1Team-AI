...
class MirrAISDPipeline:
    ...
    def run(
        ...
        def _normalize_seed_value(value: int) -> int:
            normalized = int(value) % seed_max
            return normalized if normalized > 0 else 1

        request_seed_value: Optional[int] = None
        if seed is not None:
            request_seed_value = _normalize_seed_value(int(seed))
            seed_rng = random.Random(request_seed_value)
            seeds = [
                _normalize_seed_value(request_seed_value + idx * 9973)
                for idx in range(top_k)
            ]
        elif self.config.seeds:
            seeds = [_normalize_seed_value(s) for s in self.config.seeds[:top_k]]
            seed_rng = random.Random(seeds[0] if seeds else 1)
        else:
            seed_rng = random
            seeds = [seed_rng.randint(0, seed_max) for _ in range(top_k)]
        logger.info(f"[SDPipeline] seeds={seeds}, request_seed={request_seed_value}")
        ...
        if hair_length == "short" and len(seeds) < 5:
            extra = 5 - len(seeds)
            seeds.extend(seed_rng.randint(0, seed_max) for _ in range(extra))
            logger.info(
        ...
        for rank, cand in enumerate(candidates[:requested_top_k]):
                            ...
                            if int((broad_changed_u8 > 0).sum()) >= 240:
                                ...
                                if (
                                    ...
                                    if female_short_direct_cloth_restore_px >= 120:
                                        ...
                                        strict_shoulder_lane_top = max(
                                            0,
                                            max(
                                                int(cutoff_y_for_post + face_h * 0.26),
                                                int(y2 + face_h * 0.12),
                                            ),
                                        )
                                        strict_shoulder_lane_bottom = min(H, int(y2 + face_h * 1.94))
                                        strict_shoulder_inner_x = min(
                                            W,
                                            max(
                                                int(cx + face_w * 0.42),
                                                int(x2 - face_w * 0.12),
                                        ...
                                        strict_shoulder_bright_rescue_u8 = np.zeros(final_bgr.shape[:2], dtype=np.uint8)
                                        ...
                                        if hair_length == "short":
                                            ...
                                            if (
                                                ...
                                                and strict_shoulder_inner_x < strict_shoulder_outer_x
                                            ):
                                                strict_shoulder_lane_u8[
                                                    strict_shoulder_lane_top:strict_shoulder_lane_bottom,
                                                    strict_shoulder_inner_x:strict_shoulder_outer_x,
                                                ] = 255
                                            if (
                                                subject_cloth_anchor_for_post is not None
                                                and subject_cloth_anchor_for_post.shape == final_bgr.shape[:2]
                                                ...
                                                anchor_gate_u8 = cv2.dilate(
                                                    (
                                                        np.clip(subject_cloth_anchor_for_post.astype(np.float32), 0.0, 1.0)
                                                        > 0.04
                                                    ).astype(np.uint8)
                                                    * 255,
                                                ...
                                                strict_shoulder_gate_u8 = cv2.bitwise_or(
                                                    strict_shoulder_gate_u8,
                                                    anchor_gate_u8,
                                                )
                                            if (
                                                shoulder_cloth_release_for_post is not None
                                                and shoulder_cloth_release_for_post.shape == final_bgr.shape[:2]
                                                and float(shoulder_cloth_release_for_post.sum()) > 0.0
                                            ):
                                                release_gate_u8 = cv2.dilate(
                                                    (
                                                        np.clip(shoulder_cloth_release_for_post.astype(np.float32), 0.0, 1.0)
                                                        > 0.04
                                                    ).astype(np.uint8)
                                                    * 255,
                                                    cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (31, 31)),
                                            ...
                                            if int((strict_shoulder_gate_u8 > 0).sum()) >= 120:
                                                ...
                                                if anchor_gate_u8 is not None:
                                                    strict_shoulder_restore_u8 = cv2.bitwise_or(
                                                        strict_shoulder_restore_u8,
                                                        cv2.bitwise_and(anchor_gate_u8, strict_shoulder_gate_u8),
                                                    )
                                                if release_gate_u8 is not None:
                                                    strict_shoulder_restore_u8 = cv2.bitwise_or(
                                                        strict_shoulder_restore_u8,
                                                        cv2.bitwise_and(release_gate_u8, strict_shoulder_gate_u8),
                                                    )
                                                strict_shoulder_restore_u8 = cv2.morphologyEx(
                                                    strict_shoulder_restore_u8,
                                                    cv2.MORPH_CLOSE,
                                                    cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (17, 25)),
                                                )
                                                strict_shoulder_restore_u8 = cv2.dilate(
                                                    strict_shoulder_restore_u8,
                                                    cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 15)),
                                                    iterations=1,
                                                )
                                                ...
                                                if final_hair_mask is not None and final_hair_mask.shape == final_bgr.shape[:2]:
                                                    ...
                                                    strict_shoulder_restore_u8 = cv2.bitwise_and(
                                                        strict_shoulder_restore_u8,
                                                        cv2.bitwise_not(strict_hair_guard_u8),
                                                    )
                                                if int((strict_shoulder_restore_u8 > 0).sum()) >= 120:
                                                    strict_shoulder_restore_mask = cv2.GaussianBlur(
                                                        strict_shoulder_restore_u8.astype(np.float32) / 255.0,
                                                        (0, 0),
                                                        sigmaX=2.8,
                                                        sigmaY=4.4,
                                                    ).astype(np.float32)
                                                    strict_shoulder_reference_mask = np.maximum(
                                                        female_short_cloth_reference_mask,
                                                        np.clip(
                                                            cv2.GaussianBlur(
                                                                strict_shoulder_gate_u8.astype(np.float32) / 255.0,
                                                                (0, 0),
                                                                sigmaX=3.4,
                                                                sigmaY=5.2,
                                                            ).astype(np.float32)
                                        ...
                                        if side_restore_px >= 80:
                                            ...
                                            if (
                                                ...
                                                strict_right_crop = (
                                                    strict_shoulder_inner_x,
                                                    strict_crop_top,
                                                    strict_shoulder_outer_x,
                                                    strict_crop_bottom,
                                                )
                                                for crop_left, crop_top, crop_right, crop_bottom in (strict_right_crop,):
                                                    if crop_left >= crop_right or crop_top >= crop_bottom:
                                                        continue
                                                    crop_u8 = strict_shoulder_restore_u8[
                                                        crop_top:crop_bottom,
                                                        crop_left:crop_right,
                                                    ]
                                                    if int((crop_u8 > 0).sum()) < 60:
                                                        continue
                                                    crop_mask = strict_shoulder_restore_mask[
                                                        crop_top:crop_bottom,
                                                        crop_left:crop_right,
                                                    ]
                                                    crop_reference_mask = np.clip(
                                                        strict_shoulder_reference_mask[
                                                            crop_top:crop_bottom,
                                                            crop_left:crop_right,
                                                        ].astype(np.float32),
                                                        0.0,
                                                    ...
                                                    crop_bright_rescue_u8 = np.zeros(
                                                        (crop_h, crop_w),
                                                        dtype=np.uint8,
                                                    )
                                                    if crop_h >= 36 and crop_w >= 36:
                                                        rescue_lane_u8 = np.zeros(
                                                            (crop_h, crop_w),
                                                            dtype=np.uint8,
                                                        )
                                                        ...
                                                        if (
                                                            rescue_lane_top < rescue_lane_bottom
                                                            and rescue_lane_left < rescue_lane_right
                                                        ):
                                                            rescue_lane_u8[
                                                                rescue_lane_top:rescue_lane_bottom,
                                                                rescue_lane_left:rescue_lane_right,
                                                            ] = 255
                                                            ...
                                                            crop_sat = cv2.cvtColor(
                                                                crop_rgb,
                                                                cv2.COLOR_RGB2HSV,
                                                            )[:, :, 1].astype(np.float32)
                                                            ref_sat = cv2.cvtColor(
                                                                crop_reference_rgb,
                                                                cv2.COLOR_RGB2HSV,
                                                            )[:, :, 1].astype(np.float32)
                                                            crop_diff = np.mean(
                                                            ...
                                                            crop_bright_rescue_u8 = cv2.bitwise_and(
                                                                crop_bright_rescue_u8,
                                                                rescue_lane_u8,
                                                            )
                                                            crop_bright_rescue_u8 = cv2.bitwise_and(
                                                            ...
                                                            if crop_reference_mask.shape == (crop_h, crop_w):
                                                                crop_ref_gate_u8 = (
                                                                    (
                                                                        np.clip(
                                                                            crop_reference_mask.astype(np.float32),
                                                                            0.0,
                                                                            1.0,
                                                                        )
                                                                        > 0.05
                                                            ...
                                                            if crop_hair_mask is not None and crop_hair_mask.shape == (crop_h, crop_w):
                                                                crop_hair_guard_u8 = cv2.dilate(
                                                                    (
                                                                        np.clip(
                                                                            crop_hair_mask.astype(np.float32),
                                                                            0.0,
                                                                            1.0,
                                                                        )
                                                                        > 0.16
                                                                ...
                                                                crop_bright_rescue_u8 = cv2.bitwise_and(
                                                                    crop_bright_rescue_u8,
                                                                    cv2.bitwise_not(crop_hair_guard_u8),
                                                                )
                                                            crop_bright_rescue_u8 = cv2.morphologyEx(
                                                                crop_bright_rescue_u8,
                                                                cv2.MORPH_OPEN,
                                                                cv2.getStructuringElement(
                                                                    cv2.MORPH_ELLIPSE,
                                                            ...
                                                            crop_bright_rescue_u8 = cv2.dilate(
                                                                crop_bright_rescue_u8,
                                                                cv2.getStructuringElement(
                                                                    cv2.MORPH_ELLIPSE,
                                                                    (7, 9),
            ...
            if (
                ...
                try:
                    ...
                    under_jaw_generation_reference_rgb = img_rgb
                    under_jaw_generation_reference_source = "raw_source"
                    ...
                    if hair_length == "short" and "source_cloth_reference_rgb" in locals():
                        if (
                            ...
                            under_jaw_generation_reference_rgb = source_cloth_reference_rgb
                            under_jaw_generation_reference_source = "conditioned_source_cloth"
                    ...
                    if debug_data_common is not None and rank == 0 and hair_length == "short":
                        debug_data_common["under_jaw_generation_reference_source"] = under_jaw_generation_reference_source
                    ...
                    if under_jaw_cloth_refine_px >= 140:
                        ...
                        if hair_length == "short":
                            short_under_jaw_generation_silhouette_mask = self._build_short_cloth_generation_silhouette_mask(
                                current_rgb=final_rgb,
                                source_rgb=under_jaw_generation_reference_rgb,
                                cloth_mask=cloth_reference_mask,
                                face_bbox=face_bbox,
                                cutoff_y=cutoff_y_for_post,
                                protect_mask=protect_mask_for_sd,
                        ...
                        if hair_length == "short":
                            ...
                            seed=int(cand["seed"]) + 1871,
                                reference_rgb=under_jaw_generation_reference_rgb,
                        ...
                        final_rgb = self._stabilize_under_jaw_cloth_fill(
                            final_rgb,
                            under_jaw_generation_reference_rgb,
                            under_jaw_generation_mask,
                            cloth_reference_mask,
                            hair_length=hair_length,
                        )
