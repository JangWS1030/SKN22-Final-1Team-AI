...
if [ "${ENABLE_STARTUP_GIT_PULL_LOWER}" = "1" ] || \
   ...
   [ "${ENABLE_STARTUP_GIT_PULL_LOWER}" = "on" ]; then
    ...
    if [ -n "${GITHUB_TOKEN}" ]; then
        FETCH_LOG="$(mktemp)"
        RESET_LOG="$(mktemp)"
        if git -C /app fetch --depth 1 "https://${GITHUB_TOKEN}@github.com/PracLee/hair_swap_model.git" "${GIT_PULL_REF}" >"${FETCH_LOG}" 2>&1 \
            && git -C /app reset --hard FETCH_HEAD >"${RESET_LOG}" 2>&1; then
            echo "[entrypoint_sd] git pull success"
        else
            echo "[warn] git pull failed, using bundled code"
        fi
        rm -f "${FETCH_LOG}" "${RESET_LOG}"
    else
