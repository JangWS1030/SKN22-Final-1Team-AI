...
def format_summary(endpoint: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "workerImages": sorted({worker.get("imageName") for worker in endpoint.get("workers", [])}),
    }
...
def main() -> int:
    ...
    parser.add_argument("--name-suffix", default=None, help="Optional suffix appended to current endpoint name")
    parser.add_argument("--delete-old", action="store_true", help="Delete the old endpoint after recreation")
    parser.add_argument("--drain-source-first", action="store_true", help="Scale source endpoint down before recreation")
    parser.add_argument("--wait-workers", action="store_true", help="Wait until recreated endpoint has a worker")
    ...
    new_name = args.new_name or (f"{source['name']}{args.name_suffix}" if args.name_suffix else None)
    ...
    new_template = create_template(api_key, clone_template_payload(source_template, template_name))
