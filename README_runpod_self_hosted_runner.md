...
현재 워크플로우는 `ubuntu-latest`를 사용합니다. RunPod self-hosted runner로 전환하려면 아래 파일들의 `runs-on`을 다음처럼 바꾸면 됩니다.
...
- `runs-on`을 self-hosted 라벨로 바꾼 뒤 코드 push 또는 `Run workflow` 시 self-hosted runner가 잡을 가져감
